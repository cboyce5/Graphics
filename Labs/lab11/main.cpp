...
void myTimer(int value){
	...
	for (int i = 0; i < numBalls; i++) {
		if (balls[i]->location.getX() > groundSize) {
			...
			Vector normal(-1,0,0);
			Vector d = balls[i]->direction;
			balls[i]->direction = d - (2*dot(d,normal)*normal);
		}
		else if (balls[i]->location.getX() < -groundSize) {
			balls[i]->moveBackward();
			Vector normal(1,0,0);
			Vector d = balls[i]->direction;
			balls[i]->direction = d - (2*dot(d,normal)*normal);
		}
		else if (balls[i]->location.getZ() > groundSize) {
			balls[i]->moveBackward();
			Vector normal(0,0,-1);
			Vector d = balls[i]->direction;
			balls[i]->direction = d - (2*dot(d,normal)*normal);
	...
	for (int i = 0; i < numBalls; i++) {
		for (int j = 0; j < numBalls; j++) {
			if (i != j) {	//if the indeces are not the same (i.e. dont check the same ball against itself)
				if ( (balls[i]->location - balls[j]->location).mag() < (balls[i]->radius + balls[j]->radius) ) { 	//if the distance between the two balls is less than the sum of the radii
					balls[i]->moveBackward();
					balls[j]->moveBackward();	//move each ball backwards to 'unstick' them
					
					Vector normalI = (balls[j]->location - balls[i]->location);	
					Vector normalJ = (balls[i]->location - balls[j]->location);	//compute normal vectors using location of each ball
					
					Vector dI = balls[i]->direction;
					Vector dJ = balls[j]->direction;	//temp direction variables to make direction calculation more readable
					
					balls[i]->direction = dI - (2*dot(dI,normalI)*normalI);
					balls[j]->direction = dJ - (2*dot(dJ,normalJ)*normalJ);	//using same equation for recalculating direction using normal vectors
					
					balls[i]->moveForward();
					balls[j]->moveForward();	//move each ball forward once since we moved backwards, but along the new direction
				}
			}
		}
	}
