...
void renderScene(void)  {
	...
	positionListener(cameraX,cameraY,cameraZ,cameraDirX,cameraDirY,cameraDirZ,0,1,0);
...
void normalKeysDown( unsigned char key, int x, int y )  {
    ...
    if( key == 'w' || key == 'W' ) {
		...
		//sounds runs when w or s is pressed
		ALenum sourceState;
...
void initializeOpenAL( int argc, char *argv[] ) {
	...
	alutInit( &argc, argv );
	device = alcOpenDevice( NULL ); // open default
