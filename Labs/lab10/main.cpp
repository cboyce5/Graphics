...
void initScene() {
    ...
    for(int i = 0; i < numSprites; i++) {
		...
		int x = rand() % numSprites;
		x -= numSprites/2;
		int z = i-numSprites/2;
		Point p = Point(x,0,z);
		treeSpritePositions.push_back(p);
    }
...
void renderScene(void) {
	...
	Point XZcamera = Point(cameraXYZ.getX(),0.,cameraXYZ.getZ());
	...
	for (int i = 0; i < treeSpritePositions.size(); i++) {
		Vector spriteVector = treeSpritePositions.at(i) - XZcamera;
	...
	while(distances.size() != 0 ) {
		...
		t.push_back(treeSpritePositions.at(index));
		treeSpritePositions.erase(treeSpritePositions.begin()+index);
		distances.erase(distances.begin()+index);
	}
	
	
	treeSpritePositions = t;
	

    // TODO #3: Compute rotation of tree to face camera
	Vector a = Vector(0,0,1);
	Vector b = Vector(-cameraXYZ.getX(),0.,-cameraXYZ.getZ());
	b.normalize();
	double angle;
    for(int i = 0; i < treeSpritePositions.size(); i++ ) {
        glPushMatrix(); {
			...
			//cout << angle << " " << cameraXYZ.getX() << " " << treeSpritePositions.at(i).getX() << endl;
