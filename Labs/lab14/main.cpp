...
init (const char *filename, const char *animfile)
{
    ...
    // TODO #6 - Setup Framebuffer.  Use the doc - https://blackboard.mines.edu/bbcswebdav/courses/Fall.2016.CSCI441A/docs/FBOs.pdf
...
void display () {
	...
	glPushMatrix(); {
		 ...
		 glUseProgram( blurShaderProgramHandle );
			 GLint fsize = glGetUniformLocation(blurShaderProgramHandle, "framebufferSize");
			 GLint bsize = glGetUniformLocation(blurShaderProgramHandle, "blurSize");
			 glUniform1f(fsize,framebufferWidth);
			 glUniform1f(bsize,BLUR_SIZE);
		 glBegin(GL_QUADS); {
...
int main( int argc, char *argv[] ) {
    ...
    glutCreateWindow ("Lab14 - MD5 Model w/ VBO & FBO");
