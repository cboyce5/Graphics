...
void renderScene(void)  {
	...
	//glUseProgram(shaderProgramHandle);
	ps.draw(camera,particleAge,shaderProgramHandle);
	//glUseProgram(0);
...
int main(int argc, char **argv) {
	...
	readPPM("textures/brick.ppm",imageWidth,imageHeight,data);
	registerOpenGLTexture(data,imageWidth,imageHeight,ground);
	
	//transparent loading/saving
	int rgbWidth, rgbHeight, alphaWidth, alphaHeight, channels;
    unsigned char *treeTexData, *treeMaskData;
	treeTexData = SOIL_load_image( "textures/raindrop.jpg",
                                  &rgbWidth, &rgbHeight, &channels,
                                  SOIL_LOAD_AUTO );
	printf( "[INFO]: read in image snowflake.jpg of size %d x %d with %d colors\n", rgbWidth, rgbHeight, channels );
    treeMaskData = SOIL_load_image( "textures/raindropAlpha.jpg",
                                   &alphaWidth, &alphaHeight, &channels,
                                   SOIL_LOAD_AUTO );
	printf( "[INFO]: read in image snowflake.jpg of size %d x %d with %d colors\n", alphaWidth, alphaHeight, channels );
    registerTransparentOpenGLTexture( treeTexData, treeMaskData, rgbWidth, rgbHeight, particleHandle );
	printf( "[INFO]: transparent texture read in and registered\n" );

	}
	
	if (argc != 2) {
		fprintf(stdout, "Please enter a control file for a particle system.\n");
		exit(0);
	}
	else {
		setupShaders();
		ps = ParticleSystem(argv[1],particleHandle);
	}
